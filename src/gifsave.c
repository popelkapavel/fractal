...
inline Word HASH(Word index,Word lastbyte) {
...
static int InputByte(void) {
    ...
    ret = Image[(ImageTop+RelPixY)*ImageBpl+ImageLeft+RelPixX];
    if(ImageMap) ret=ImageMap[ret];
...
int GIF_CompressImage(int left, int top, int width, int height,unsigned char *image,int bpl,unsigned char *map)
{
     ...
     */
    RelPixX = RelPixY = 0;
    Image=image;ImageBpl=bpl;ImageMap=map;
